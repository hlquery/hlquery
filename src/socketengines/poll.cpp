...
void SocketEngine::Deinit()
{
    ...
    if (Instance && Instance->Logs && Instance->Logs->GetDebugMode())
    {
        Instance->Logs->Debug("socketengine", 
            "Deinit: Clearing " + std::to_string(PollFDs.size()) + " registered file descriptors.");
    }
...
bool SocketEngine::AddFD(EventHandler* EH, int EventsMask)
{
    ...
    {
        Instance->Logs->Debug("socketengine", 
            "AddFD: Registered fd=" + std::to_string(fd) + 
            " with EventsMask=0x" + std::to_string(EventsMask) + 
            " (poll events=0x" + std::to_string(pfd.events) + 
...
void SocketEngine::DelFD(EventHandler* EH)
{
    ...
    {
        Instance->Logs->Debug("socketengine", 
            "DelFD: Removing fd=" + std::to_string(fd) + 
            " (PollFDs.size()=" + std::to_string(PollFDs.size()) + ").");
    }
...
int SocketEngine::DispatchEvents()
{
    ...
    {
        Instance->Logs->Debug("socketengine", 
            "DispatchEvents: ENTRY (PollFDs.size()=" + std::to_string(PollFDs.size()) + 
            ", ShuttingDown=" + std::to_string(ShuttingDown) + 
            ", ForceExit=" + std::to_string(ForceExit) + ").");
    }
    ...
    {
        /*
         * Block indefinitely when idle - same as epoll
         * This prevents unnecessary CPU wakeups every 10ms when idle
         */
    ...
    {
        Instance->Logs->Debug("socketengine", 
            "DispatchEvents: About to call poll() with " + std::to_string(PollFDs.size()) + 
            " fds, timeout=" + (timeout_ms == -1 ? std::string("infinite") : std::to_string(timeout_ms) + "ms") + ".");
    }
    ...
    {
        Instance->Logs->Debug("socketengine", 
            "poll() returned " + std::to_string(nfds) + 
            " events (timeout=" + (timeout_ms == -1 ? std::string("infinite") : std::to_string(timeout_ms) + "ms") + 
            ", PollFDs.size()=" + std::to_string(PollFDs.size()) + 
            (nfds < 0 ? ", errno=" + std::string(strerror(errno)) : "") + ".");
    }
    ...
    if (nfds == 0)
    {
        ...
        {
            Instance->Logs->Debug("socketengine", 
                "poll() timeout: nfds=0, timeout_ms=" + 
                (timeout_ms == -1 ? std::string("infinite") : std::to_string(timeout_ms) + "ms") + 
                ", HasPendingWork()=" + std::string(HasPendingWork() ? "true" : "false") + ".");
        }
    ...
    if (nfds > 0)
    {
        ...
        {
            Instance->Logs->Debug("socketengine", 
                "Processing " + std::to_string(nfds) + " events from poll() - iterating through " + 
                std::to_string(PollFDs.size()) + " file descriptors.");
        }
        ...
        for (size_t i = 0; i < PollFDs.size(); ++i)
        {
            ...
            if (Instance && Instance->Logs && Instance->Logs->GetDebugMode())
            {
                ...
                if (pfd.revents & POLLIN) event_types += "POLLIN ";
                if (pfd.revents & POLLOUT) event_types += "POLLOUT ";
                if (pfd.revents & POLLERR) event_types += "POLLERR ";
                if (pfd.revents & POLLHUP) event_types += "POLLHUP ";
                if (pfd.revents & POLLNVAL) event_types += "POLLNVAL ";
                if (pfd.revents & POLLPRI) event_types += "POLLPRI ";

                Instance->Logs->Debug("socketengine", 
                    "poll() detected event on fd=" + std::to_string(pfd.fd) + 
                    ", revents=0x" + std::to_string(pfd.revents) + 
                    " (" + (event_types.empty() ? "unknown" : event_types) + ").");
            }
        ...
        for (size_t i = 0; i < PollFDs.size(); ++i)
        {
            ...
            if (pfd.revents & POLLIN)
            {
                ...
                try
                {
                    ...
                    {
                        Instance->Logs->Debug("socketengine", 
                            "Calling OnEventHandlerRead() for fd=" + std::to_string(pfd.fd) + ".");
                    }
                    ...
                    {
                        Instance->Logs->Debug("socketengine", 
                            "OnEventHandlerRead() completed for fd=" + std::to_string(pfd.fd) + 
                            ", events_processed=" + std::to_string(events_processed) + ".");
                    }
                }
                catch (...)
                {
        ...
        if (nfds > 0 && events_processed == 0 && Instance && Instance->Logs && Instance->Logs->GetDebugMode())
        {
            ...
            Instance->Logs->Debug("socketengine", 
                "poll() returned " + std::to_string(nfds) + " events but processed 0 (fds_with_events=" + 
                std::to_string(fds_with_events) + ", PollFDs.size()=" + std::to_string(PollFDs.size()) + ").");
        }
    }
