...
/* Ultra-High Performance epoll engine with advanced optimizations */
...
void SocketEngine::Init() 
{
     ...
     * If EpollFD is not -1, we assume it's already initialized and skip.
     */
    ...
    if (EpollFD == -1)
    {
        std::cerr << "SocketEngine::Init() failed: epoll_create1() returned " << EpollFD
...
void SocketEngine::StartIOWorkerThreads()
{
    ...
    for (unsigned int i = 0; i < NumWorkers; ++i)
    {
        ...
        if (ThreadLimit::GetCurrentThreadCount() >= ThreadLimit::GetMaxThreads())
        {
            if (Instance && Instance->Logs)
            {
                Instance->Logs->Normal("socketengine", 
                    "Thread limit reached, stopping I/O worker thread creation at " + std::to_string(i) + ".");
            }
...
bool SocketEngine::AddFD(EventHandler* EH, int EventsMask)
{
    ...
    {
        Instance->Logs->Debug("socketengine", 
            "AddFD: ENTRY (FD=" + std::to_string(EH->HasFD() ? EH->GetFD() : -1) + 
            ", EventsMask=0x" + std::to_string(EventsMask) + 
            ", EpollFD=" + std::to_string(EpollFD) + 
            ", valid=" + std::string(EpollFDValid.load() ? "true" : "false") + ").");
    }
    ...
    if (epoll_ctl(EpollFD, EPOLL_CTL_ADD, fd, &ev) == -1)
    {
        ...
        {
            Instance->Logs->Debug("socketengine", 
                "AddFD: FAILED - epoll_ctl error (fd=" + std::to_string(fd) + 
    ...
    {
        Instance->Logs->Debug("socketengine", 
            "AddFD: SUCCESS - fd=" + std::to_string(fd) + 
            " added to epoll (ActiveConnections=" + 
            std::to_string(ActiveConnections.load()) + ").");
    }
...
void SocketEngine::DelFD(EventHandler* EH)
{
    if (EpollFD == -1 || !EpollFDValid.load())
    {
        ...
        {
            Instance->Logs->Debug("socketengine", 
                "DelFD: Engine down, skipping (EpollFD=" + std::to_string(EpollFD) + 
                ", valid=" + std::string(EpollFDValid.load() ? "true" : "false") + ").");
        }
    ...
    if (EH->HasFD())
    {
        ...
        {
            Instance->Logs->Debug("socketengine", 
                "DelFD: Removing fd=" + std::to_string(fd) + " from epoll.");
        }
        ...
        if (result == 0)
        {
            ...
            {
                Instance->Logs->Debug("socketengine", 
                    "DelFD: Successfully removed fd=" + std::to_string(fd) + 
                    " from epoll (ActiveConnections=" + 
                    std::to_string(ActiveConnections.load() - 1) + ").");
            }
        ...
        else if (saved_errno == EBADF)
        {
            ...
            {
                Instance->Logs->Debug("socketengine", 
                    "DelFD: EBADF error, marking EpollFD as invalid (fd=" + std::to_string(fd) + ").");
            }
        ...
        else if (saved_errno == ENOENT)
        {
            ...
            {
                Instance->Logs->Debug("socketengine", 
                    "DelFD: ENOENT - fd=" + std::to_string(fd) + " was not in epoll (already removed?).");
            }
    ...
    else
    {
        ...
        {
            Instance->Logs->Debug("socketengine", 
                "DelFD: Handler has no FD, skipping.");
        }
    }
}
...
int SocketEngine::DispatchEvents()
{
    ...
    {
        Instance->Logs->Debug("socketengine", 
            "DispatchEvents: ENTRY (EpollFD=" + std::to_string(EpollFD) + 
            ", valid=" + std::string(EpollFDValid.load() ? "true" : "false") + 
            ", ShuttingDown=" + std::string(ShuttingDown ? "true" : "false") + 
            ", ForceExit=" + std::string(ForceExit ? "true" : "false") + ").");
    }
    ...
    if (EpollFD == -1 || !EpollFDValid.load())
    {
        ...
        {
            Instance->Logs->Debug("socketengine", 
                "DispatchEvents: Engine deinitialized, returning 0.");
        }
    ...
    if (ShuttingDown || ForceExit)
    {
        ...
        {
            Instance->Logs->Debug("socketengine", 
                "DispatchEvents: Shutdown requested, returning 0.");
        }
    ...
    {
        /*
         * Block indefinitely when idle instead of 10ms timeout.
         * The 10ms timeout was causing DispatchEvents to be called 100 times per second
         * even when idle, wasting CPU. Instead, block indefinitely and rely on shutdown
         * signal handling (SIGINT/SIGTERM) for responsive shutdown.
         * 
         * If we need shutdown responsiveness, we can use a signal handler or timer,
         * but not a constant 10ms wakeup loop.
         */

        timeout_ms = -1; /* Block indefinitely when idle - no CPU waste */
    ...
    {
        Instance->Logs->Debug("socketengine", 
            "DispatchEvents: Calling epoll_wait(timeout=" + 
            (timeout_ms == -1 ? std::string("infinite") : std::to_string(timeout_ms) + "ms") + ").");
    }
    ...
    if (local_EpollFD == -1 || !EpollFDValid.load())
    {
        ...
        {
            Instance->Logs->Debug("socketengine", 
                "DispatchEvents: EpollFD became invalid before epoll_wait, returning 0.");
        }
    ...
    if (nfds == MAX_EVENTS)
    {
        ...
        {
            Instance->Logs->Debug("socketengine", 
                "epoll_wait returned MAX_EVENTS (" + std::to_string(MAX_EVENTS) + 
                ") - may have more events pending, will process in loop.");
        }
    }
    ...
    {
        Instance->Logs->Debug("socketengine", 
            "DispatchEvents: epoll_wait returned " + std::to_string(nfds) + 
            " events (errno=" + (nfds < 0 ? std::string(strerror(errno)) : "0") + ").");
    }
    ...
    if (nfds < 0)
    {
        ...
        else if (err != EINTR)
        {
            ...
            {
                Instance->Logs->Debug("socketengine", 
                    "DispatchEvents: epoll_wait error, returning 0 (errno=" + 
    ...
    if (nfds == 0)
    {
        ...
        {
            Instance->Logs->Debug("socketengine", 
                "DispatchEvents: epoll_wait timeout, no events, returning 0.");
        }
    ...
    if (nfds > 0)
    {
        ...
        if (auto* inst = Instance.get(); should_flush && inst && inst->Database)
        {
            try
            {
                inst->Database->Flush();
    ...
    for (int i = 0; i < nfds && i < static_cast<int>(Events.size()); ++i)
    {
        ...
        if (ev & (EPOLLHUP | EPOLLRDHUP | EPOLLERR))
        {
            ...
            if (Instance && Instance->Logs && Instance->Logs->GetDebugMode())
            {
                ...
                Instance->Logs->Debug("socketengine", 
                    "DispatchEvents: Processing error/hangup event (" + event_types + 
                    ", ev=0x" + std::to_string(ev) + ", fd=" + (valid_fd ? std::to_string(fd) : "INVALID") + 
                    ") - cleaning up connection immediately.");
            }
            ...
            if (valid_fd && EH->HasFD())
            {
                ...
                catch (...)
                {
                    ...
                    {
                        Instance->Logs->Normal("socketengine", 
                            "DispatchEvents: Exception during error handler cleanup for fd=" + 
                            std::to_string(fd) + ".");
                    }
                }
            }
    ...
    for (int i = 0; i < nfds && i < static_cast<int>(Events.size()); ++i)
    {
        ...
        if (fd < 0 || fd > MAX_REASONABLE_FD)
        {
            ...
            {
                Instance->Logs->Normal("socketengine", 
                    "DispatchEvents: Invalid fd detected in read event (fd=" + std::to_string(fd) + 
                    ", ptr=" + std::to_string(reinterpret_cast<uintptr_t>(EH)) + ") - skipping.");
            }
        ...
        if (ev & EPOLLIN)
        {
            ...
            if (!EH->HasFD() || EH->GetFD() != fd)
            {
                ...
                {
                    Instance->Logs->Normal("socketengine", 
                        "DispatchEvents: Handler fd changed during read processing (was " + 
                        std::to_string(fd) + ") - skipping.");
                }
            ...
            catch (...)
            {
                ...
                {
                    Instance->Logs->Normal("socketengine", 
                        "DispatchEvents: Exception in OnEventHandlerRead() for fd=" + std::to_string(fd) + 
                        " - cleaning up.");
                }
    ...
    for (int i = 0; i < nfds && i < static_cast<int>(Events.size()); ++i)
    {
        ...
        if (fd < 0 || fd > MAX_REASONABLE_FD)
        {
            ...
            {
                Instance->Logs->Normal("socketengine", 
                    "DispatchEvents: Invalid fd detected in write event (fd=" + std::to_string(fd) + 
                    ", ptr=" + std::to_string(reinterpret_cast<uintptr_t>(EH)) + ") - skipping.");
            }
        ...
        if (ev & EPOLLOUT)
        {
            ...
            {
                Instance->Logs->Debug("socketengine", 
                    "DispatchEvents: Processing write event #" + std::to_string(write_events) + 
                    " (fd=" + std::to_string(fd) + ").");
            }
            ...
            if (!EH->HasFD() || EH->GetFD() != fd)
            {
                ...
                {
                    Instance->Logs->Normal("socketengine", 
                        "DispatchEvents: Handler fd changed during write processing (was " + 
                        std::to_string(fd) + ") - skipping.");
                }
            ...
            catch (...)
            {
                ...
                {
                    Instance->Logs->Normal("socketengine", 
                        "DispatchEvents: Exception in OnEventHandlerWrite() for fd=" + std::to_string(fd) + 
                        " - cleaning up.");
                }
            ...
            {
                Instance->Logs->Debug("socketengine", 
                    "DispatchEvents: Write event #" + std::to_string(write_events) + " completed.");
            }
        }
    }
    ...
    {
        Instance->Logs->Debug("socketengine", 
            "DispatchEvents: Processed " + std::to_string(read_events) + " read events, " + 
            std::to_string(write_events) + " write events (total_events=" + std::to_string(nfds) + ").");
    }
    ...
    if (nfds == MAX_EVENTS)
    {
        ...
        while (loop_count < MAX_LOOPS && total_processed < MAX_TOTAL_EVENTS)
        {
            ...
            if (more_events <= 0)
            {
                ...
                if (more_events < 0 && errno != EINTR)
                {
                    ...
                    {
                        Instance->Logs->Normal("socketengine", 
                            "epoll_wait error in overflow loop: " + std::string(strerror(errno)) + ".");
                    }
                }
            ...
            {
                Instance->Logs->Debug("socketengine", 
                    "Found " + std::to_string(more_events) + " additional events after hitting limit (loop " + 
                    std::to_string(loop_count + 1) + ").");
            }
        ...
        if (loop_count >= MAX_LOOPS)
        {
            ...
            {
                Instance->Logs->Normal("socketengine", 
                    "Hit max loops processing overflow events - may have more pending.");
            }
        }
        ...
        if (total_processed >= MAX_TOTAL_EVENTS)
        {
            ...
            {
                Instance->Logs->Normal("socketengine", 
                    "Hit max total events limit - throttling to prevent overload.");
            }
        }
