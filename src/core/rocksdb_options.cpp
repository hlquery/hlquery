...
namespace HLManager 
{
     ...
     RocksDBOptions RocksDBOptions::LoadFromConfigReader(const ConfigReader& ReaderInstance) 
     {
          ...
          auto RocksDBSettingsTag = ReaderInstance.GetTag("rocksdb");
          ...
          if (Instance && Instance->Logs) 
          {
               ...
               Instance->Logs->Normal("rocksdb_config", 
                   "RocksDB options loaded: write_buffer=" + std::to_string(OptionsResult.WriteBufferSize / (1024 * 1024)) + "MB, " +
                   "block_cache=" + std::to_string(OptionsResult.BlockCacheSize / (1024 * 1024)) + "MB, " +
                   ThreadInfoStr + ", compression=" + OptionsResult.Compression + ".");
          }
