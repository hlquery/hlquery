...
CONFIGURE_COMMAND := ./configure '--skip-tests' '--engine=epoll'
HLQUERY_VERSION := 1.0.0-rc6
...
CONFIGURE_CXXFLAGS = -std=c++17 -O2 -fPIC  $(CPPFLAGS)
CONFIGURE_CFLAGS = -std=c11 -O2 -fPIC  $(CPPFLAGS)
...
CONFIGURE_LDFLAGS = -ldl $(FS_LIB) -pthread -lssl -lcrypto $(EXTRA_LDFLAGS)
...
EXTRA_MODULE_SIMPLE_SRCS := 
EXTRA_MODULE_DIR_SRCS := 
EXTRA_MODULE_LIBS := 
EXTRA_MODULE_KNOWN_NAMES := m_analytics
EXTRA_MODULE_ENABLED_NAMES := 
...
$(CONFIG_HEADER): configure make/config.h.tpl
	@echo "$(CYAN)Regenerating $(CONFIG_HEADER) via ./configure '--skip-tests' '--engine=epoll'...$(NC)"
	@./configure '--skip-tests' '--engine=epoll' >/dev/null
...
clean:
	...
	@rm -f $(RUN_DIR)/hlquery 2>/dev/null || true
...
PREFIX ?= /home/cferry/p/hlquery/hlquery
CONFDIR ?= /home/cferry/p/hlquery/hlquery/run/conf
LOGDIR ?= /home/cferry/p/hlquery/hlquery/run/logs
DATADIR ?= /home/cferry/p/hlquery/hlquery/run/data
RUNDIR ?= /home/cferry/p/hlquery/hlquery/run/pid
BINDIR ?= /home/cferry/p/hlquery/hlquery/run/bin
SYSTEMD_UNIT_DIR ?= 
INSTALL ?= install
...
install:
	...
	@if [ -f "$(HLQUERY_WRAPPER_TEMPLATE)" ]; then \
		sed \
			-e 's|$${HLQUERY_VERSION}|$(HLQUERY_VERSION)|g' \
			-e 's|$${HLQUERY_CONFIG_DIR}|$(CONFDIR)|g' \
			-e 's|$${HLQUERY_DATA_DIR}|$(DATADIR)|g' \
			-e 's|$${HLQUERY_LOG_DIR}|$(LOGDIR)|g' \
			-e 's|$${HLQUERY_PID_DIR}|$(RUNDIR)|g' \
			-e 's|$${HLQUERY_BIN_DIR}|$(BINDIR)|g' \
			"$(HLQUERY_WRAPPER_TEMPLATE)" > "$(STAGED_RUN_DIR)/hlquery"; \
		chmod 0755 "$(STAGED_RUN_DIR)/hlquery"; \
		echo "$(BLUE)   Wrapper: $(STAGED_RUN_DIR)/hlquery$(NC)"; \
	else \
		echo "$(YELLOW)   Warning: $(HLQUERY_WRAPPER_TEMPLATE) not found; wrapper was not installed.$(NC)"; \
	fi
