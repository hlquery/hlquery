...
EXTRA_MODULE_DIR_SRCS := $(SRC_DIR)/modules/extra/m_analytics/analyticsmanager.cpp $(SRC_DIR)/modules/extra/m_analytics/m_analytics.cpp
EXTRA_MODULE_LIBS := $(RUN_DIR)/modules/m_analytics.so
EXTRA_MODULE_KNOWN_NAMES := m_analytics
EXTRA_MODULE_ENABLED_NAMES := m_analytics
...
$(RUN_DIR)/modules/extra.so: MODULE_EXTRA_LDFLAGS += -lssl -lcrypto
$(RUN_DIR)/modules/m_analytics.so: MODULE_EXTRA_LDFLAGS += -lssl -lcrypto

# Explicit build rules for enabled extra modules injected by configure.
$(RUN_DIR)/modules/m_analytics.so: $(OBJ_DIR)/modules/extra/m_analytics/analyticsmanager.module.o $(OBJ_DIR)/modules/extra/m_analytics/m_analytics.module.o $(ROCKSDB_LIB) | $(BIN_DIR)
	@mkdir -p $(RUN_DIR)/modules
	@echo "$(CYAN)Linking extra module m_analytics...$(NC)"
	$(CXX) -shared -o $@ $^ $(CONFIGURE_LDFLAGS) $(MODULE_SHARED_LDFLAGS) $(MODULE_EXTRA_LDFLAGS)
